...
while not app.need_exit():
    ...
    if candidates:
        ...
        # ===== 瓶底基准颜色（瓶子最下方中心区域）=====
        base_x = x + w // 2
        base_y = y + h - h // 8                         # 瓶底上方 1/8 处
        base_m = w // 6
        base_r = base_g = base_b = 0
        base_n = 0
        for sx, sy in [(base_x, base_y),
                       (base_x - base_m, base_y),
                       (base_x + base_m, base_y)]:
            ...
            base_b += v & 0xFF
            base_n += 1
        base_r //= base_n; base_g //= base_n; base_b //= base_n
        ...
        if roi_lh > 30:
            ...
            if blobs:
                ...
                # 精修：在 rough_y 上下 40px 范围内找最大亮度梯度
                scan_top = max(roi_ly, rough_y - 30)
                scan_bot = min(roi_ly + roi_lh, rough_y + 30)
                mid_x = x + w // 2
                max_diff = 0
                best_row = rough_y
                ...
                if max_diff > 10:                          # 梯度阈值 15→10
                    # 颜色权重验证：液面下方颜色需与瓶底基准相近
                    check_y = best_row + 10
                    if check_y < y + h:
                        ck_x = x + w // 2
                        val = img.get_pixel(ck_x, check_y)
                        if isinstance(val, (list, tuple)) and len(val) >= 1: v = val[0]
                        elif isinstance(val, int): v = val
                        else: v = 0
                        cr = (v >> 16) & 0xFF
                        cg = (v >> 8) & 0xFF
                        cb = v & 0xFF
                        cd = ((cr - base_r)**2 + (cg - base_g)**2 + (cb - base_b)**2) ** 0.5
                        if cd < 150:                   # 颜色差异阈值 120→150
                            level_y = best_row
    ...
    img_disp.save("/root/photo/1.jpg")
    disp.show(img_disp)
