...
while not app.need_exit():
    ...
    if has_obj:
        ...
        if _prev_level_y > 0 and _prev_level_y < y + h:
            base_r=base_g=base_b=base_cnt=0; base_m=w//3
            for sr in range(_prev_level_y+5, min(y+h, _prev_level_y+60), 8):
                for sc in range(base_x-base_m, base_x+base_m, 12):
                    val=img.get_pixel(sc,sr)
                    if isinstance(val,(list,tuple)) and len(val)>=1: v=val[0]
                    elif isinstance(val,int): v=val
                    else: continue
                    base_r+=(v>>16)&0xFF; base_g+=(v>>8)&0xFF; base_b+=v&0xFF; base_cnt+=1
            if base_cnt>5: base_r//=base_cnt; base_g//=base_cnt; base_b//=base_cnt
            else: base_r=base_g=base_b=0
        else:
            base_y=y+h-h//10; base_r=base_g=base_b=0; base_m=w//4
            for sx,sy in [(base_x,base_y),(base_x-base_m,base_y),(base_x+base_m,base_y),(base_x,base_y+5)]:
                val=img.get_pixel(sx,sy)
                if isinstance(val,(list,tuple)) and len(val)>=1: v=val[0]
                elif isinstance(val,int): v=val
                else: v=0
                base_r+=(v>>16)&0xFF; base_g+=(v>>8)&0xFF; base_b+=v&0xFF
        ...
        if roi_lh > 30:
            ...
            if blobs:
                ...
                for row in range(scan_top, scan_bot, 2):
                    ...
                    w_grad = 0.3 + (1.0-contrast)*2.5
                    w_col  = 0.5 + contrast*3.0
                    w_con  = 0.3 + contrast*2.0

                    score = diff_sum*w_grad + below_score*120*w_col + continuity*15 + consistency*100*w_con + gradient_bonus*80
                    if score > best_score: best_score=score; best_row=row
                if best_score > 10: level_y = best_row
