    ...
    if candidates_filtered:
        ...
        if _obj_locked is not None:
            ...
            if dx < _obj_lock_window and dy < _obj_lock_window and dw < _obj_lock_window and dh < _obj_lock_window:
                ...
                # 超限：采样锁定框周围颜色判断物体是否真变了
                color_changed = False
                # 在锁定框中心 + 四边取色，和当前帧同位置比较
                sample_pts = [(lx+lw//2, ly+lh//2),
                              (lx+lw//2, ly-5), (lx+lw//2, ly+lh+5),
                              (lx-5, ly+lh//2), (lx+lw+5, ly+lh//2)]
                pt_diff = 0
                for sx, sy in sample_pts:
                    if sx < 0 or sy < 0 or sx >= CAM_W or sy >= CAM_H: continue
                    v0 = img.get_pixel(sx, sy)
                    v1 = img.get_pixel(
                        sx - lx + x, sy - ly + y) if 0 <= sx-lx+x < CAM_W and 0 <= sy-ly+y < CAM_H else None
                    # 简单灰度比较
                    if isinstance(v0, (list, tuple)) and len(v0) >= 1: v0 = v0[0]
                    elif isinstance(v0, int): pass
                    else: v0 = 0
                    if isinstance(v1, (list, tuple)) and len(v1) >= 1: v1 = v1[0]
                    elif isinstance(v1, int): pass
                    else: v1 = 0
                    if isinstance(v0, int) and isinstance(v1, int):
                        g0 = ((v0>>16)&0xFF) + ((v0>>8)&0xFF) + (v0&0xFF)
                        g1 = ((v1>>16)&0xFF) + ((v1>>8)&0xFF) + (v1&0xFF)
                        pt_diff += abs(g0 - g1)
                if pt_diff > 150:             # 颜色变化大 → 物体真变了
                    color_changed = True

                if color_changed:
                    _obj_lock_count = max(0, _obj_lock_count - 1)
                else:
                    _obj_lock_count = max(0, _obj_lock_count)  # 保持

                if _obj_lock_count <= -_outlier_tolerance:
                    _obj_locked = None
                else:
                    x, y, w, h = lx, ly, lw, lh
        else:
