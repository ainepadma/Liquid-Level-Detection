...
# ========== 检测/锁定参数（瓶子/杯子） ==========
# 检测阈值
DETECT_CONF_TH = 0.22           # 模型A基础阈值
DETECT_IOU_TH = 0.45
DETECT_B_CONF_SCALE = 0.50      # 模型B阈值倍率

# 杯子/瓶子分开阈值
CUP_CONF_MIN = 0.15
BOTTLE_CONF_MIN = 0.22
    ...
    if has_obj:
        ...
        if roi_lh > 30:
            ...
            if blobs:
                ...
                rough_y = best_blob.y()
                scan_top = max(roi_ly, rough_y-12); scan_bot = min(roi_ly+roi_lh, rough_y+12)
                mid_x = x + w // 2; area_cx = x + w // 2; area_w = min(w//4, 300)
